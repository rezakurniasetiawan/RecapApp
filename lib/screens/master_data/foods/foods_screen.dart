...
class _FoodScreenState extends State<FoodScreen> {
  ...
  saveDataToStorage() async {
    ...
    await storage.write(key: 'master-food', value: jsonEncode(tmp));
    _getDataFromStorage();
  }

  _getDataFromStorage() async {
    String? data = await storage.read(key: 'master-food');
    if (data != null) {
      ...
      if (dataDecoded is List) {
        setState(() {
          ...
          for (var item in dataDecoded) {
            listCountry.add(CountryModel.fromJson(item));
            print(listCountry);
          }
        });
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Food'), actions: [
        IconButton(
          icon: Icon(
            Icons.add,
